<?php
return [
    /**
 * NOTICE OF LICENSE.
 *
 * NOBS Community Edition is open-sourced software licensed under the GNU Affero General Public License v3.0
 * The details is bundled with this project in the file LICENSE.txt.
 *
 * @project    NOBS Community Edition
 *
 * @author     HDVinnie <hdinnovations@protonmail.com>
 * @license    https://www.gnu.org/licenses/agpl-3.0.en.html/ GNU Affero General Public License v3.0
 */
    'about' => '在这里，您可以查看用户自定义喜欢的播放列表！',
    'attached-success' => '种子已成功附加到您的播放列表中。',
    'add-to-playlist' => '把种子加入播放列表',
    'add-torrent' => '添加种子',
    'added-by' => '来自:',
    'cover' => '选择一个封面图片',
    'create' => '创建新的播放列表',
    'delete-playlist' => '删除播放列表',
    'deleted' => '已删除播放列表！',
    'detached-success' => '种子已成功从您的播放列表中移除。',
    'desc' => '播放列表描述',
    'download-all' => '下载全部播放列表的种子',
    'edit-playlist' => '编辑播放列表',
    'list-by' => '列表源自于:',
    'list-about' => '关于列表:',
    'is-private' => '私人播放列表？',
    'playlists' => '播放列表',
    'private-error' => '这是一个私人播放列表！您无权访问其他用户的私人播放列表！',
    'published-success' => '您的播放列表已成功创建！',
    'title' => '标题',
    'titles' => '标题',
    'update-success' => '您的播放列表已成功更新！',
    'playlist-suggestions' => '播放列表建议',
    'last-addition-at' => '最后添加于',
    'playlist-suggestion-rejections' => '不推荐播放列表',
    'suggest-torrent' => '推荐种子',
    'suggestion-review' => '您的建议已发送给播放列表创建者进行审核',
    'suggestion-approved' => '播放列表建议已获批准',
    'suggestion-rejected' => '播放列表建议被拒绝',
];
