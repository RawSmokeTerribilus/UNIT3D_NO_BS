...
Route::middleware('language')->group(function (): void {
    ...
    */
    Route::middleware(['auth', 'banned', 'verified'])->group(function (): void {
        ...
        // Arcade — ScummVM Gaming
        // gaming.isolation (COOP + COEP) only on the show route — the index page
        // does not run Wasm/SharedArrayBuffer and does not need cross-origin isolation.
        // Applying it to the index breaks bfcache and adds ~10 s to every navigation.
        ...
        Route::prefix('api/saves')->name('gaming.saves.')
            ->middleware('throttle:'.GlobalRateLimit::GAMING_SAVES->value)
            ->withoutMiddleware('throttle:'.GlobalRateLimit::WEB->value)
            ->group(function (): void {
                Route::get('/manifest/{gameId}', [App\Http\Controllers\Api\GameSaveController::class, 'manifest'])->name('manifest')->where('gameId', '[a-zA-Z0-9\-]+');
                Route::post('/sync', [App\Http\Controllers\Api\GameSaveController::class, 'sync'])->name('sync');
                Route::get('/download/{gameId}/{filename}', [App\Http\Controllers\Api\GameSaveController::class, 'download'])->name('download')->where('gameId', '[a-zA-Z0-9\-]+');
            });
