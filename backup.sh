...
echo "💾 Volcando DB..."
docker exec unit3d-staging-db mysqldump -u "$DB_USER" -p"$DB_PASS" --no-tablespaces "$DB_NAME" > "$SNAPSHOT_DIR/db_unit3d.sql" 2>/dev/null
...
tar -czf "$SNAPSHOT_DIR/unit3d_staging_full_$NOW.tar.gz" \
    --exclude='./backups' \
