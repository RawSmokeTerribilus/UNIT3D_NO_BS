#!/bin/bash
===============================================================================
UNIT3D Auto-Installer v1.2 (Fixed)
For UNIT3D Community Edition v9.2.0 (Laravel 12)
...
check_os() {
	...
	case $VERSION in
		"22.04")
			PHP_VERSION="8.4"
			log_info "Detected Ubuntu 22.04 - Will install PHP $PHP_VERSION"
			;;
		"24.04")
			PHP_VERSION="8.4"
			log_info "Detected Ubuntu 24.04 - Will install PHP $PHP_VERSION"
			;;
		*)
...
install_unit3d() {
	...
	log_info "Checking out v9.2.0..."
	git checkout v9.2.0 2>/dev/null || git checkout main
	log_info "Checked out UNIT3D version: $(git describe --tags 2>/dev/null || git rev-parse --short HEAD)"
