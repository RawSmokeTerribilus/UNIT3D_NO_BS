...
IP_MALAS=$(docker exec unit3d-app php artisan tinker --execute="echo DB::table('failed_login_attempts')->select('ip_address')->groupBy('ip_address')->havingRaw('COUNT(*) > 3')->pluck('ip_address')->implode(PHP_EOL);" 2>/dev/null | grep -vE '^(192\.168|10|172\.(1[6-9]|2[0-9]|3[0-1])|127)\.')
