    ...
    public function register(): void
    {
        ...
        $this->app->instance(LoginResponse::class, new class () implements LoginResponse {
            public function toResponse($request): \Illuminate\Http\RedirectResponse
            {
                ...
                // Upgrade stored http:// intended URLs when the app is HTTPS
                $intended = session()->get('url.intended');

                if (is_string($intended) && str_starts_with($intended, 'http://') && str_starts_with(config('app.url'), 'https://')) {
                    session(['url.intended' => 'https' . substr($intended, 4)]);
                }
        ...
        $this->app->instance(TwoFactorLoginResponse::class, new class () implements TwoFactorLoginResponse {
            public function toResponse($request): \Illuminate\Http\RedirectResponse|\Illuminate\Contracts\Support\Responsable
            {
                $path = session()->pull('url.intended', Fortify::redirects('home'));

                // Upgrade stored HTTP intended URLs when the app is HTTPS
                if (is_string($path) && str_starts_with($path, 'http://') && str_starts_with(config('app.url'), 'https://')) {
        ...
        Fortify::authenticateUsing(function (Request $request): \Illuminate\Database\Eloquent\Model {
            ...
            if ($password === false) {
                defer(function () use ($user, $request): void {
                    ...
                    if ($otherUsernamesAttempted->count() > 3) {
                    ...
                    if ($ipAttemptCount > 15) {
                        BlockedIp::upsert([[
                            'ip_address' => $ip,
                            'user_id'    => User::SYSTEM_USER_ID,
                            'reason'     => 'Brute-force attempt: 15 failed attempts in last 24h',
                            'expires_at' => now()->addHours(1),
