...
return [
    ...
    'permissions-policy' => [
        ...
        // ambient-light-sensor: descartado por Chrome (Permissions-Policy no lo reconoce).
        // battery, document-domain, execution-while-not-rendered, execution-while-out-of-viewport
        // y navigation-override están deprecated o nunca formaron parte estándar de
        // Permissions-Policy — se eliminaron para evitar warnings de cabecera.
    ...
    'csp' => [
        ...
        'font-src' => [
            ...
            'schemes' => [
                'data:', // ScummVM Wasm embeds fonts as data: URIs for its GUI
                // 'https:'
            ],
        ...
        'img-src' => [
            ...
            'host' => [
                '*.tmdb.org',
                '*.themoviedb.org',
                '*.ytimg.com', // Para las miniaturas de los vídeos de YouTube
                '*.googleusercontent.com', // Por si usas avatares de Google
            ],
        ...
        'script-src' => [
            ...
            'allow' => [
                // Necesario para compilar el motor WebAssembly de ScummVM.
                // 'wasm-unsafe-eval' SOLO permite compilar binarios .wasm —
                // NO habilita eval() ni new Function() en JavaScript.
                "'wasm-unsafe-eval'",
            ],
        ...
        // Necesario para ScummVM WebAssembly: el motor Emscripten crea Workers
        // mediante new Worker(URL.createObjectURL(blob)) — Chrome exige blob: explícito.
        'worker-src' => [
            'self' => true,
